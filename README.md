...
* If body sizes of HTTP requests are not too big, as those could lead to [denial-of-service (DoS) attacks](https://en.wikipedia.org/wiki/Denial-of-service_attack)
* If non-secure HTTP methods are used (i.e. `TRACE` or `DEBUG`), as those lead to several [information disclosures](https://portswigger.net/kb/issues/00500a00_http-trace-method-is-enabled)
* All the tests mentioned above at once
...
* [**Examples**](#examples): A few examples of outputs are shown in this sections.
* [**Going further**](#going-further): LoggyLogs has been designed as an open source project since day 1. This section includes:
    ...
    2. [Improvements](#improvements): Adds that could be made to make LoggyLogs even more efficient
...
* Run LoggyLogs for the first time: `python loggylogs.py --help`
...
* Put the logs to analyze here:
    * A file example named `samples.log` is already present
    * Feel free to replace it
    * In case the analysis of several files is required, change the `LOGS_PATH` constant in `loggylogs.py`
...
## Examples
Once LoggyLogs is set, it can parse and eventually detect suspicious requests with the following command:
...
* Examples:
    * Generate the documentation for the `Exporter()` class: `python -m pydoc lib/parser.py`
...
## License
[LoggyLogs](https://github.com/sljrobin/LoggyLogs) is licensed under the MIT License ([Source](https://github.com/sljrobin/LoggyLogs/blob/main/LICENSE)).